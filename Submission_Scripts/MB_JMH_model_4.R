...
train_test$Jeff_2 <- paste(train_test$X0,train_test$X1,train_test$X2,train_test$X3,sep='_')
train_test$Jeff_3 <- paste(train_test$X0,train_test$X1,train_test$X2,train_test$X3,train_test$X5,train_test$X6,train_test$X8,sep='_')
train_test$Jeff_4 <- paste(train_test$X0,train_test$X5,train_test$X8,sep='_')
train_test$Jeff_5 <- paste(train_test$X0,train_test$X5,sep='_')
train_test$Jeff_6 <- paste(train_test$X0,train_test$X8,sep='_')
train_test$Jeff_7 <- paste(train_test$X0,train_test$X1,train_test$X5,train_test$X8,sep='_')
#colnames(train_test[,379:384])
for(i in c(seq.int(2,9,1),379:384)){
...
train_test <- one_hot(train_test,cols=colnames(train_test[,2:9, with=FALSE]),dropCols = TRUE)
#colnames(train_test)
#colnames(train_test[,582:587, with=FALSE])
train_test <- one_hot(train_test,cols=colnames(train_test[,582:587, with=FALSE]),dropCols = TRUE)
...
train_valid_w_features <- cbind(y=train$y,train_valid_w_features)
      ...
      # #Looking to understand X3=c values only
      # train_valid_w_features <- train_valid_w_features[X3=='c']
      # View(train_valid_w_features[,120:150]) #check
...
y <- "y"
x <- c("X314","X261","X127","X29","X263","X279","X315","X118","X232","X119","X313","X311","X54","X115","X76","X136","X47","X316","X189","X234","X272")
#x <- colnames(train_model[,c(11:378), with=FALSE])
...
#h2o.saveModel(gbm_model_3,"gbm_model_4")
...
y <- "y"
x <- colnames(train_model[,11:378, with=FALSE])
...
#h2o.saveModel(rf_model_3,"rf_model_4")
#path <- getwd()
...
valid_model$Ensemble_model_3 <- 0.2*valid_model$gbm_model_3 + 0.8*valid_model$rf_model_3
...
valid_model$Ensemble_model_3 <- 0.9985*valid_model$Ensemble_model_3
R2(valid_model$Ensemble_model_3,valid_model$y) # 0.5618969
...
h2o.loadModel("F:\\kaggle\\Mercedes-Benz\\gbm_model_4\\gbm_model_3") #gbm_model_3
gbm_model_3 <- h2o.getModel("gbm_model_3")
test_pred_y_gbm_model_3 <- as.data.frame(h2o.predict(gbm_model_3,test_h2o))[,1]

h2o.loadModel("F:\\kaggle\\Mercedes-Benz\\rf_model_4\\rf_model_3") #rf_model_3
gbm_model_3 <- h2o.getModel("rf_model_3")
test_pred_y_rf_model_3 <- as.data.frame(h2o.predict(rf_model_3,test_h2o))[,1]


test_pred_y <- 0.9985*( 0.2*test_pred_y_gbm_model_3 + 0.8*test_pred_y_rf_model_3)



#Create Submission File
submission_name <- "MB_JMH_Submission_4"  #Only need to change in this one place in future code versions
...
min(get(submission_name)$y) # 76.50288
max(get(submission_name)$y) # 128.1815
mean(get(submission_name)$y) # 100.8892

#Output Submission File
write.csv(get(submission_name),paste(submission_name,'.csv',sep=''), row.names = FALSE)

#Leaderboard score = 0.55227
