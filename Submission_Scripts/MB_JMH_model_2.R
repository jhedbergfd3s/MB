...
gbm_model_2 <- h2o.gbm(training_frame = train_model_h2o,      ## H2O frame holding the training data
                       ...
                       model_id="gbm_model_2",              ## internal H2O name for model
                       ntrees = 720,                  ## use fewer trees than default (50) to speed up training
...
#summary(gbm_model_2)
(h2o.r2(gbm_model_2,train=T)) # 0.5859611
(h2o.r2(gbm_model_2,valid=T)) # 0.5604289
#h2o.saveModel(gbm_model_2,"gbm_model_2")
#h2o.performance(gbm_model_2, train_model_h2o)
#h2o.performance(gbm_model_2, valid_model_h2o)
...
h2o.loadModel("F:\\kaggle\\Mercedes-Benz\\gbm_model_2\\gbm_model_2") #gbm_model_2
gbm_model_2 <- h2o.getModel("gbm_model_2")
test_pred_y <- as.data.frame(h2o.predict(gbm_model_2,test_h2o))[,1]

#Create Submission File
submission_name <- "MB_JMH_Submission_2"  #Only need to change in this one place in future code versions
...
min(get(submission_name)$y) # 78.16973
max(get(submission_name)$y) # 121.1851
mean(get(submission_name)$y) # 101.0341

#Output Submission File
write.csv(get(submission_name),paste(submission_name,'.csv',sep=''), row.names = FALSE)

#Leaderboard score = 0.55682
